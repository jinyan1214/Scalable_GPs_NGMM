# see https://github.com/f-t-s/cholesky_by_KL_minimization/blob/master/src/Cholesky.jl
...
function _innerprod(
    range1, range2, nzind::Array{Int64,1}, nzval::Array{Float64,1}
)
    iter1 = range1.start
    u1 = range1.stop
...
#This function computes the in place incomplete Cholesky factorisation using 
#an inner product wrapped in a function.
function _el_icholRWHL!(
    s::Array{Float64}, ind::Array{Int64,1}, jnd::Array{Int64,1}
)
    @inbounds begin
        #piv_i goes from 1 to N and designates the number, which is being treated 
        for piv_i in 1:(size(ind, 1) - 1)
            #piv_j ranges over pointers and designates where to find the present column
            for piv_j in ind[piv_i]:(ind[piv_i + 1] - 1)
                #iter designates the pointer, at which the i-iterator starts
                iter::Int64 = ind[piv_i]
                #jter designates the pointer, at which the j-iterator starts
                jter::Int64 = ind[jnd[piv_j]]
...
function icholGPU!(U::SparseMatrixCSC{Float64,Int64})
    ...
    return U.nzval .= Array{Float64}(d_U.nzVal)
end
...
function IChol(L, U)
    return IChol(
        L,
        U,
        zeros(eltype(L), size(L, 1)),
        Vector{eltype(L)}(undef, size(L, 1)),
    )
end
...
function logdet(A::NoiseCov)
    return 2 * logdet(UpperTriangular(A.U)) -
           2 * logdet(UpperTriangular(A.L)) + sum(log.(A.invσ))
end
