...
function _create_U_indices(
    supernodes::AbstractVector{<:IndexSuperNode{Ti}}
) where {Ti}
    I = Ti[]
    J = Ti[]
...
function factorize(
    ...
    supernodal_assignment::IndirectSupernodalAssignment{Ti,Tm};
    nugget=0.0,
) where {Tv,Ti,Tm}
    # Determines the maximum amount of space needed 
    maximum_buffer_L_size =
        maximum((maximum.(size.(supernodal_assignment.supernodes, 1))))^2
    maximum_buffer_U_size = maximum(
        maximum.(
            size.(supernodal_assignment.supernodes, 1) .*
            size.(supernodal_assignment.supernodes, 2)
        ),
    )
    maximum_buffer_m_size = maximum(
        maximum.(size.(supernodal_assignment.supernodes, 1))
    )
    # The buffer used to store the lower triangular Cholesky factor that is computed as part of the update
    buffer_L = [Vector{Tv}(undef, maximum_buffer_L_size) for k in 1:nthreads()]
    # The buffer used to store the columns, in order to to allow for a batched forward/back substitution
    buffer_U = [Vector{Tv}(undef, maximum_buffer_U_size) for k in 1:nthreads()]
    # The buffer that is used to store the measurements the of supernode
    buffer_m = [Vector{Tm}(undef, maximum_buffer_m_size) for k in 1:nthreads()]
    ...
    @threads for node in supernodal_assignment.supernodes
        ...
        local_buffer_L = reshape(
            view(buffer_L[threadid()], 1:(n_rows^2)), n_rows, n_rows
        )
        local_buffer_U = reshape(
            view(buffer_U[threadid()], 1:(n_rows * n_columns)),
            n_rows,
            n_columns,
        )
        local_buffer_U .= 0
        ...
        for (k, index) in enumerate(column_indices(node))
            ...
            U.nzval[range_of_nnzs] .= local_buffer_U[
                1:length(range_of_nnzs), k
            ]
            @assert node.row_indices[length(range_of_nnzs)] ==
                index ==
                U.rowval[range_of_nnzs[end]]
        end
    end
    return U
end
