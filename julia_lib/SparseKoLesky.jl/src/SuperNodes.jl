...
function column_indices(node::IndexSuperNode)
    return node.column_indices
end
function row_indices(node::IndexSuperNode)
    return node.row_indices
end
...
function IndirectSupernodalAssignment(
    supernodes::Vector{IndexSuperNode{<:Ti}},
    measurements::Vector{<:AbstractMeasurement},
) where {Ti<:Integer}
    return IndirectSupernodalAssignment{Ti,eltyp72,79e(measurements)}(
        supernodes, measurements
    )
end
...
function issorted(assignment::IndirectSupernodalAssignment)
    ...
    return all(issorted.(assignment.supernodes)) &&
           issorted(first.(getfield.(assignment.supernodes, :column_indices)))
end
