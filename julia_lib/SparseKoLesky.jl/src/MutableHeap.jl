...
struct MutableHeap{Tn<:AbstractNode,Tid}
    ...
    function MutableHeap{Tn,Tid}(
        nodes::Vector{Tn}, lookup::Vector{Tid}
    ) where {Tn<:AbstractNode,Tid}
        return if !all(getval.(nodes) .> typemin(vtype(Tn)))
            error("typemin of value type reserved")
        else
            new{Tn,Tid}(nodes, lookup)
        end
    end
end
...
function _swap!(h, a, b)
    ...
    tempNode = h.nodes[a]
    h.nodes[a] = h.nodes[b]
    return h.nodes[b] = tempNode
end

#Node comparisons
import Base.isless
function isless(a::RankedNode, b::RankedNode)
    return isless((-a.rank, a.val), (-b.rank, b.val))
end
...
function >=(a::RankedNode, b::RankedNode)
    return (-a.rank, a.val) >= (-b.rank, b.val)
end
