...
function Δ∇δPointMeasurement(in::PointMeasurement{d}) where {d}
    return Δ∇δPointMeasurement{Float64,d}(
        in.coordinate,
        zero(Float64),
        SVector{d,Float64}(zeros(Float64, d)),
        one(Float64),
    )
end

function ΔδPointMeasurement(in::PointMeasurement{d}) where {d}
    return ΔδPointMeasurement{Float64,d}(
        in.coordinate, zero(Float64), one(Float64)
    )
end
...
function point_measurements(x::Matrix; dims=1)
    ...
    d = size(x, 1)
    return [
        PointMeasurement{d}(SVector{d,Float64}(x[:, k])) for k in 1:size(x, 2)
    ]
end
