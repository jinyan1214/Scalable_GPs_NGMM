...
function predictSkeleton!(
    μ, σ, L, B, y::AbstractVector, α, β, γ, δ, ℓ, skel, Kij!, ITest
)
    NTest = length(ITest)
    ...
    for k_par in length(skel.parents):-1:1
        k = findlast(x -> x == skel.parents[k_par], skel.children)
        ...
        for j in 1:NTest
            ℓ[j] =
                -B[k, j] * (one(eltype(ℓ)) + β[j] / (δ[j] - β[j])) / δ[j] /
                γ[j]
        end
    ...
    reverse!(skel.parents)
    return reverse!(skel.children)
end

# Version for multiple samples and therefore matrix-valued y
function predictSkeleton!(
    μ, σ, L, B, y::AbstractMatrix, α, β, γ, δ, ℓ, skel, Kij!, ITest
)
    NTest = length(ITest)
    NTrain = length(skel.children)
    NSamples = size(y, 2)
    ...
    for k_par in length(skel.parents):-1:1
        ...
        for j in 1:NTest
            for k_sample in 1:NSamples
                μ[j, k_sample] +=
                    ℓ[j] / γ[j] *
                    (y[k, k_sample] + B[k, j] * α[j, k_sample] / (δ[j] - β[j]))
            end
        end
        @. σ += ℓ^2
        old_k = k
...
function predict_threaded(
    skeletons::Vector{Skeleton{Ti}},
    ITest::AbstractVector{Ti},
    yTrain::AbstractMatrix,
    Kij!,
) where {Ti}
    ...
    GC.@preserve LBuffer BBuffer yBuffer Threads.@threads for skel in skeletons
        ...
        L = unsafe_wrap(
            Array{Float64,2},
            pointer(LBuffer, lin_inds_L[1, Threads.threadid()]),
            (nChildren, nChildren),
        )
        B = unsafe_wrap(
            Array{Float64,2},
            pointer(BBuffer, lin_inds_B[1, Threads.threadid()]),
            (nChildren, NTest),
        )
        y = unsafe_wrap(
            Array{Float64,2},
            pointer(yBuffer, lin_inds_y[1, Threads.threadid()]),
            (nChildren, NSamples),
        )
