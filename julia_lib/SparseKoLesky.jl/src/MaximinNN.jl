...
function _update_distances!(
    nearest_distances::AbstractVector, id, new_distance
)
...
function _update_distances!(
    nearest_distances::AbstractMatrix, id, new_distance
)
...
function _construct_initial_distance(
    tree::Union{KDTree,BallTree}, x, k_neighbors
)
    if length(tree.data) >= k_neighbors
        ...
        return vcat(
            fill(typemax(eltype(out_distances)), (k_neighbors - k_max)),
            out_distances,
        )
    end
end
...
function concatenate_ordering(
    P_vec::AbstractVector{<:AbstractVector{<:Integer}}
)
    offset = 0
...
function maximin_ordering(
    ...
    init_distances=[
        fill(typemax(eltype(eltype(x))), (size(xₖ, 2))) for xₖ in x
    ],
    Tree=KDTree,
)
    for (k, xₖ) in enumerate(x)
        ...
        for l in (k + 1):length(x)
            init_distances[l] .=
                min.(
                    init_distances[l], _construct_initial_distance(tree, x[l])
                )
        end
    end
...
function maximin_ordering(
    ...
    k_neighbors;
    init_distances=[
        fill(typemax(eltype(eltype(x))), (k_neighbors, size(xₖ, 2))) for
        xₖ in x
    ],
    Tree=KDTree,
)
    for (k, xₖ) in enumerate(x)
        ...
        for l in (k + 1):length(x)
            init_distances[l] .=
                min.(
                    init_distances[l],
                    _construct_initial_distance(tree, x[l], k_neighbors),
                )
        end
    end
    ...
    for (k, xₖ) in enumerate(x)
        # create the ordering of k-th set of points
        Pₖ, ℓₖ = maximin_ordering(
            xₖ, k_neighbors; init_distances=init_distances[k]
        )
        push!(P, Pₖ)
        push!(ℓ, ℓₖ)
    end
...
function _gather_assignments(assignments, first_parent)
    ...
    ranges = [
        (first_indices[k]:(first_indices[k + 1] - 1)) for
        k in 1:(length(first_indices) - 1)
    ]
...
function supernodal_reverse_maximin_sparsity_pattern(
    x::AbstractMatrix,
    P,
    ℓ,
    ρ;
    ...
    while last_parent < N
        ...
        last_aggregation_point =
            findnext(
                l -> (l == N + 1) || (ℓ_temp[l] < α * ρ * min_ℓ),
                1:(N + 1),
                last_aggregation_point,
            ) - 1
        ...
        last_parent = findnext(
            l -> (l + 1 == N + 1) || ℓ[l + 1] < min_ℓ, 1:N, first_parent
        )
        ...
        for column_indices in column_indices_list
            ...
            for column_index in column_indices
                ...
                new_row_indices = inrange(
                    children_tree, x[:, column_index], ρ * ℓ[column_index]
                )
                new_row_indices = new_row_indices[findall(
                    new_row_indices .<= column_index
                )]
...
function ordering_and_sparsity_pattern(
    x::AbstractMatrix,
    ρ;
    init_distances=fill(typemax(eltype(x)), (size(x, 2))),
    lambda=1.5,
    alpha=1.0,
    Tree=KDTree,
)
...
function ordering_and_sparsity_pattern(
    ...
    P, ℓ = maximin_ordering(x; init_distances, Tree)
    supernodes = supernodal_reverse_maximin_sparsity_pattern(
        reduce(hcat, x), P, ℓ, ρ; lambda, alpha, Tree
    )
    return P, ℓ, supernodes
end
...
function ordering_and_sparsity_pattern_DiracsFirstThenUnifScale(
    x::AbstractVector{<:AbstractMatrix},
    ρ,
    k_neighbors;
    init_distances=[
    ...
    P_all[(N_boundary + 1):end] = reduce(
        vcat,
        [
            P[(N_boundary + 1):(N_boundary + N_domain)] .+ (i - 1) * N_domain
            for i in 1:(lm - 1)
        ],
    )
    ...
    supernodes = supernodal_reverse_maximin_sparsity_pattern(
        reduce(hcat, x), P_all, ℓ_all, ρ; lambda, alpha, Tree
    )
