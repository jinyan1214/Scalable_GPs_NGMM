...
create_table_scec_data = """CREATE TABLE IF NOT EXISTS data_sa_scec (
                                    'scen_id'          integer,
                                    'rup_var_id'       integer,
                                    'period'           real,
                                    'psa'              real,
...
create_table_res_data = """CREATE TABLE IF NOT EXISTS data_sa_res (
                                    'scen_id'          integer,
                                    'rup_var_id'       integer,
                                    'period'           real,
                                    'gmm'              text,
...
for j ,per in enumerate(per2process):
    ...
    for l, s_id in tqdm(enumerate(df_sta_metadata.site_id.values), 
        ...
        for k , gmm_key in enumerate(gmm_dict):
            ...
            if k == 0:
                df_scec_data_res[['scen_id','rup_var_id','period','psa']].to_sql('data_sa_scec',      con=db_gm_cnx, index=False, if_exists='append', method='multi', chunksize=5000)
                ...
                time_s = timeit.default_timer()
            df_scec_data_res[['scen_id','rup_var_id','gmm','period','res']].to_sql('data_sa_res', con=db_gm_cnx, index=False, if_exists='append', method='multi', chunksize=5000)
