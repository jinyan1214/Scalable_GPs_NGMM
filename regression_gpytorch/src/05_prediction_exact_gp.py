...
if __name__ == '__main__':
    ...
    for site_id in unique_site_ids:
        ...
        for row_i, row in testing_data_site.iterrows():
            ...
            conditional_source_mask = source_dist < (source_corr_threshold)
            if np.sum(conditional_source_mask) == 0:
                eq_var_id = row['eq_var_id']
                print(f"Warning: No conditional points found for site {site_id}, eq_var_id {eq_var_id}.")
                num_conditional = np.min(5000, len(conditional_x_site))
                row_indices = np.random.choice(conditional_x_site[0], size=num_conditional, replace=False)
                conditional_x = conditional_x_site[row_indices]
                conditional_y = conditional_y_site[row_indices]
            else:
                conditional_x = conditional_x_site[conditional_source_mask]
                conditional_y = conditional_y_site[conditional_source_mask]
            print(f"Site {site_id}: Number of conditional points selected: {conditional_x.shape[0]} for eq_var_id {row['eq_var_id']}.")
